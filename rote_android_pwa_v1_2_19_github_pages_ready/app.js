...
function clearForm(){['assetId','assetDesignation','typeNotes','loadInfo','advNotes','failOther','failNotes','limNotes','thisInspection','assetTypeInput'].forEach(id=>{const el=$(id); if(el) el.value='';});
...
<div class="row" style="margin-top:10px"><label class="small" style="display:flex;align-items:center;gap:8px;margin-right:10px;"><input type="checkbox" data-inspected="${i}" ${r.inspected?"checked":""}> Inspected</label>
        ${((r.prevAdv||(r.advActions&&r.advActions.length)||(r.improvements&&r.improvements.length)) || (r.prevFail||(r.failDefects&&r.failDefects.length)||(r.defects&&r.defects.length)) || (r.prevLim||r.limNotes||(r.limitations&&r.limitations.length)))?`<div class=\"row noSelect\" style=\"gap:10px;flex-wrap:wrap;margin-top:6px\">${(r.prevAdv||(r.advActions&&r.advActions.length)||(r.improvements&&r.improvements.length))?`<button type=\"button\" class=\"tagBtn\" data-info=\"adv\" data-i=\"${i}\">View advisory</button>`:''}${(r.prevFail||(r.failDefects&&r.failDefects.length)||(r.defects&&r.defects.length))?`<button type=\"button\" class=\"tagBtn\" data-info=\"fail\" data-i=\"${i}\">View fail</button>`:''}${(r.prevLim||r.limNotes||(r.limitations&&r.limitations.length))?`<button type=\"button\" class=\"tagBtn\" data-info=\"lim\" data-i=\"${i}\">View limitation</button>`:''}</div>`:''}
...
host.querySelectorAll('[data-info]').forEach(btn=>{
  const open = (ev)=>{ ev.preventDefault(); ev.stopPropagation();
    ...
    if(kind==='lim') msg = a.prevLim || a.limNotes || (a.limitations||[]).join('\\n') || '';
...
if(r.limitation){if(r.limNotes)lines.push(`Limitation notes: ${r.limNotes}`);
